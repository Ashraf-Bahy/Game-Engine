...
// Uniforms representing the color mixing matrix
...
void main(){
    ...
    float mixedRed   = dot(red, vec4(fs_in.color, 1.0));
    float mixedGreen = dot(green, vec4(fs_in.color, 1.0));
    float mixedBlue  = dot(blue, vec4(fs_in.color, 1.0));
