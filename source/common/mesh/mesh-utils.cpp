...
our::Mesh *our::mesh_utils::loadGLTF(const std::string &filename)
{
    ...
    for (size_t meshIndex = 0; meshIndex < model.meshes.size(); ++meshIndex)
    {
        ...
        for (size_t primitiveIndex = 0; primitiveIndex < mesh.primitives.size(); ++primitiveIndex)
        {
            ...
            int positionAccessorIndex = primitive.attributes.find("POSITION") != primitive.attributes.end() ? primitive.attributes.at("POSITION") : -1;
            int normalAccessorIndex = primitive.attributes.find("NORMAL") != primitive.attributes.end() ? primitive.attributes.at("NORMAL") : -1;
            int texcoordAccessorIndex = primitive.attributes.find("TEXCOORD_0") != primitive.attributes.end() ? primitive.attributes.at("TEXCOORD_0") : -1;
            int colorAccessorIndex = primitive.attributes.find("COLOR_0") != primitive.attributes.end() ? primitive.attributes.at("COLOR_0") : -1;
            ...
            int positionStride = positionBufferView.byteStride ? positionBufferView.byteStride : (positionAccessor.type == TINYGLTF_TYPE_VEC3 ? 3 * sizeof(float) : 0);
            ...
            if (normalAccessorIndex != -1)
            {
                ...
                normalStride = normalBufferView->byteStride ? normalBufferView->byteStride : (normalAccessor->type == TINYGLTF_TYPE_VEC3 ? 3 * sizeof(float) : 0);
            }
            ...
            if (texcoordAccessorIndex != -1)
            {
                ...
                texcoordStride = texcoordBufferView->byteStride ? texcoordBufferView->byteStride : (texcoordAccessor->type == TINYGLTF_TYPE_VEC2 ? 2 * sizeof(float) : 0);
            }
            ...
            if (colorAccessorIndex != -1)
            {
                ...
                colorStride = colorBufferView->byteStride ? colorBufferView->byteStride : (colorAccessor->type == TINYGLTF_TYPE_VEC3 ? 3 * sizeof(float) : (colorAccessor->type == TINYGLTF_TYPE_VEC4 ? 4 * sizeof(float) : 0));
            }
            ...
            for (size_t indexIndex = 0; indexIndex < indexCount; ++indexIndex)
            {
                ...
                {
                    vertexIndex = *reinterpret_cast<const uint16_t *>(&indicesBuffer.data[indicesBufferView.byteOffset + indicesAccessor.byteOffset +
                                                                                          indexIndex * sizeof(uint16_t)]);
                }
                else if (indicesAccessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT)
                {
                    vertexIndex = *reinterpret_cast<const uint32_t *>(&indicesBuffer.data[indicesBufferView.byteOffset + indicesAccessor.byteOffset +
                                                                                          indexIndex * sizeof(uint32_t)]);
                }
                else if (indicesAccessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE)
                {
                    vertexIndex = *reinterpret_cast<const uint8_t *>(&indicesBuffer.data[indicesBufferView.byteOffset + indicesAccessor.byteOffset +
